...
from .avellaneda_hedging_pair import AvellanedaHedgingPair
...
cdef class AvellanedaHedgingStrategy(StrategyBase):
    ...
    def init_params(self,
                    market_pairs: List[AvellanedaHedgingPair],
    ...
    def get_order_size_after_portfolio_ratio_limit(self, market_pair: AvellanedaHedgingPair) -> Decimal:
        return self.c_get_order_size_after_portfolio_ratio_limit(market_pair)

    def get_market_making_size(self, market_pair: AvellanedaHedgingPair, bint is_bid) -> Decimal:
        return self.c_get_market_making_size(market_pair, is_bid)

    def get_market_making_price(self, market_pair: AvellanedaHedgingPair, bint is_bid, size: Decimal) -> Decimal:
        return self.c_get_market_making_price(market_pair, is_bid, size)

    def get_adjusted_limit_order_size(self, market_pair: AvellanedaHedgingPair) -> Tuple[Decimal, Decimal]:
        return self.c_get_adjusted_limit_order_size(market_pair)

    def get_effective_hedging_price(self, market_pair: AvellanedaHedgingPair, bint is_bid, size: Decimal) -> Decimal:
        return self.c_calculate_effective_hedging_price(market_pair, is_bid, size)

    def check_if_still_profitable(self, market_pair: AvellanedaHedgingPair,
    ...
    def check_if_sufficient_balance(self, market_pair: AvellanedaHedgingPair,
        ...
        if current_hedging_price is None:
            if self._logging_options & self.OPTION_LOG_REMOVING_ORDER:
                self.log_with_clock(
                    ...
                    f"{order_price:.8g} {market_pair.maker.quote_asset} is none longer profitable. "
    ...
    cdef str c_place_order(self,
        ...
        cdef:
            ...
            object order_type = market_info.market.get_maker_order_type() if is_maker else \
                OrderType.MARKET # market_info.market.get_taker_order_type()
        if order_type is OrderType.MARKET:
            price = s_decimal_zero # s_decimal_nan
        if not self._active_order_canceling:
